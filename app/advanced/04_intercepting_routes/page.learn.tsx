// ─────────────────────────────────────────
// 파일명: page.learn.tsx
// 토픽: 04 Intercepting Routes
// 난이도: 고급
// 학습 시간: ~30분 (개념 + 공식 문서 따라하기)
// 선행 학습: 02_intermediate/07_route_groups (parallel routes)
//
// [무엇을 배우는가]
// - (.)folder 컨벤션: 같은 라우트 트리의 다른 경로를 가로채기
// - parallel routes (@slot) 와의 결합으로 모달 라우팅 구현
// - intercept 깊이 표기: (.) 같은 레벨, (..) 한 단계 위, (...) 루트
//
// [흔한 함정]
// - intercept 가 동작하려면 부모 layout 에 @modal 같은 슬롯이 있어야 함
// - 새로고침/직접 URL 진입은 intercept 가 안 됨 (route handler 가 본문 페이지로 매칭)
// - 모달 닫기는 router.back() 또는 modal 슬롯의 default.tsx 로 처리
// ─────────────────────────────────────────
